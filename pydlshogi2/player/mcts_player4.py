...
class MCTSPlayer(BasePlayer):
    ...
    # デフォルトチェックポイント
    DEFAULT_MODELFILE = 'checkpoints/checkpoint1375_1687.pth'
    ...
    def count_captures(self):
        try:
            board_representation = str(self.root_board)  # 盤面情報を文字列として取得
            pih1, pih2 = 0, 0  # 先手と後手の持ち駒の総数
            # 先手の持ち駒をカウント
            hand_plus_lines = re.findall(r'^P\+.*$', board_representation, re.MULTILINE)
            for hand_plus_line in hand_plus_lines:
                hand_plus = re.findall(r'00[A-Z]+', hand_plus_line)
                for piece in hand_plus:
                    count = len(piece[2:]) // 2  # 各駒の出現回数をカウント
                    pih1 += count

            # 後手の持ち駒をカウント
            hand_minus_lines = re.findall(r'^P\-.*$', board_representation, re.MULTILINE)
            for hand_minus_line in hand_minus_lines:
                hand_minus = re.findall(r'00[A-Z]+', hand_minus_line)
                for piece in hand_minus:
                    count = len(piece[2:]) // 2  # 各駒の出現回数をカウント
                    pih2 += count
            return pih1, pih2
    ...
    def get_bestmove_and_print_pv(self):
        ...
        cp = 30000 if bestvalue == 1.0 else -30000 if bestvalue == 0.0 else int(-math.log(1.0 / bestvalue - 1.0) * 600)

        pv = move_to_usi(bestmove)
        ponder_move = None
        pv_node = current_node
        legal_moves = [move_to_usi(move) for move in current_node.child_move]
        ...
        pih1, pih2 = self.count_captures()

        print('info nps {} time {} nodes {} score cp {} pih1 {} pih2 {} pv {} lm {}'.format(
            int(self.playout_count / finish_time) if finish_time > 0 else 0,
            int(finish_time * 1000),
            current_node.move_count,
            cp, pih1, pih2, pv, ' '.join(legal_moves)), flush=True)
