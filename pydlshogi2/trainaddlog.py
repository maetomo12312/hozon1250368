...
# チェックポイント保存
def save_checkpoint(epoch, step, special_name=None):
    # 特別な名前が指定されていればそれを使用、なければ通常のファイル名
    if special_name:
        path = f'{special_name}.pth'
    else:
        path = args.checkpoint.format(**{'epoch': epoch, 'step': step})
...
for e in range(args.epoch):
    ...
    logging.info('epoch = {}, steps = {}, train loss avr = {:.07f}, {:.07f}, {:.07f}, test loss = {:.07f}, {:.07f}, {:.07f}, test accuracy = {:.07f}, {:.07f}'.format(
     ...
     # 5の倍数のエポックの時に特別な名前でチェックポイントを保存
    if epoch % 5 == 0 and epoch != 0:  # 5の倍数でかつ0エポック以外
            special_name = f'checkpoint-2500-2599_{epoch}'
            save_checkpoint(epoch, t, special_name=special_name)
