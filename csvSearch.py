...
def main(fileName, output_file, csv_output_file):
    BM = []
    scores = []  # スコアリストを追加
    ...
    # CSVヘッダー
    csv_headers = ["match", "turn", "cp", "previous_value", "previous_evaluation_value", "previous_poor_hand", "legal_move", "thinking_time", "long_thoughts", "first_piece_in_hand", "second_piece_in_hand"]
        ...
        for i, line in enumerate(lines):
            if "position" in line:
                ...
                if bestmove.strip() == "resign":
                    print("resign が出力されました。ループを終了します。")
                    with open(output_file, 'a', encoding='utf-8') as f:
                        f.write(f"resign が出力されました。ループを終了します。\n")
                    break
                ...
                if current_score == 0 & legal_move_count == 0:
                    break
                if index == 1:
                ...
                if before_thinking_time > 20:
                    lgtime = 1
                else:
                    lgtime = 0
                match = 1 if bestmove.replace(' ', '') == Rmove.replace(' ', '') else 0
                row_data = [match, turn, current_score, precp, prevacp, poor,legal_move_count, current_thinking_time, lgtime, pih1, pih2]
    ...
    with open(output_file, "a", encoding="UTF-8") as f:
        ...
        f.write(f"先手の平均評価値: {avg_score1:.2f}\n")
        f.write(f"後手の平均評価値: {avg_score2:.2f}\n\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("outputFile", help="Plain text output file name")
    ...
    plain_output_file = os.path.join(args.outputDir, args.outputFile)
    ...
    for filename in os.listdir(args.usi_folder):
        if filename.endswith(".usi"):
            ...
            main(file_path, plain_output_file, csv_output_file)
