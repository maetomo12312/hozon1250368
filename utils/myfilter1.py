...
for filepath in find_all_files(args.dir):
    ...
    for line in open(filepath, 'r', encoding='utf-8'):
        ...
        if line == '%TORYO':
            toryo = True
    #if not toryo or move_len <= 50 or len(rate) < 2 or min(rate.values()) < 1000 or max(rate.values()) >= 2000:
    if not toryo or move_len <= 50 or len(rate) < 2 or min(rate.values()) < 2000 or max(rate.values()) >= 4000:
        os.remove(filepath)
    else:
        kifu_count += 1
        rates.extend(rate.values())
